...
- **TODO:** The Qwen2.5 LLM used for lyric generation occasionally outputs simplified Chinese characters (e.g. `见` instead of `見`) due to its Chinese-heavy training corpus. This can cause incorrect SudachiPy readings and display issues in the generated kanji lyrics. A post-processing step to convert simplified Chinese characters to their Japanese kanji equivalents (using a mapping table or a library such as `hanziconv`) should be added to `lyricGenerator.py`.
